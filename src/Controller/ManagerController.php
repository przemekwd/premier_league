...
class ManagerController extends AbstractController
{
    ...
    {
        $fileUploader->setTargetDirectory($this->getParameter('manager_image_dir'));
        $manager = new Manager();
        $form = $this->createForm(ManagerType::class, $manager);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            if ($file = $manager->getImageFile()) {
     ...
     */
    public function edit(Request $request, Manager $manager, FileUploader $fileUploader): Response
    {
        $fileUploader->setTargetDirectory($this->getParameter('manager_image_dir'));
        if ($manager->getImage()) {
            $manager->setImageFile(
                new File($this->getParameter('manager_image_dir') . '/' . $manager->getImage())
            );
        }
        $form = $this->createForm(ManagerType::class, $manager);
        $form->handleRequest($request);
